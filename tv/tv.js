...
function startVideoKeepAlive() {
  ...
  try {
    if (!video.src) {
      video.src = WAKE_VIDEO_MP4_BASE64;
    }
    if (typeof video.play === "function") {
      video.play().catch(() => {});
    }
  } catch (e) {
    // Безопасное подавление при ограничениях медиа-движка
  }
  ...
  if (typeof document.addEventListener === "function") {
    ...
    document.addEventListener("keydown", onUserInteraction, { once: false, passive: true });
    document.addEventListener("pointerdown", onUserInteraction, { once: false, passive: true });
  }
}
...
function initCursorAutoHide() {
  ...
  const showCursorAndScheduleHide = () => {
    if (document.body) {
  ...
  window.addEventListener("mousemove", showCursorAndScheduleHide, { passive: true });
  window.addEventListener("pointermove", showCursorAndScheduleHide, { passive: true });
  window.addEventListener("touchstart", showCursorAndScheduleHide, { passive: true });
  window.addEventListener("keydown", showCursorAndScheduleHide, { passive: true });

  // Скрыть курсор через 2 секунды после инициализации
  cursorTimeout = setTimeout(hideCursor, 2000);
}
