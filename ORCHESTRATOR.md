...
#### [ ] TASK-11.2. Миллисекундная точность паузы (ликвидация скачка 2:33 ➔ 2:34)
* **Что делаем:** Хранить точный остаток времени на паузе в миллисекундах (`table.remainingMs`), устранить рассинхрон `Math.floor` / `Math.ceil`. При возобновлении: `levelEndsAt = Date.now() + remainingMs`.
* **Файлы:** `dealer/dealer.js`, `tv/tv.js`

#### [ ] VERIFY-11.2. Тест точности паузы и монотонности таймера
* **Проверка:** Автотест `tests/test_timer_pause_precision.js`.
