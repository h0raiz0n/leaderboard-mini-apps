# 🧭 ORCHESTRATOR: Генеральный протокол качества 9.5/10 и архитектурный план
Покерная экосистема «Атмосфера» (ТВ-дашборд 4K, Telegram Mini App ведущего, бот авторизации, Firebase RTDB, Google Таблицы, Google Apps Script, Vercel Edge)

---

## 1. Архитектура системы и карта узлов

```mermaid
graph TD
    subgraph "Управление доступом и ведущими"
        A["Таблица Google: Лист «Ведущие»"] -->|"Автосинхронизация"| B["Google Apps Script + Firebase RTDB"]
        C["Пользователь Telegram"] -->|"Команда /start"| D["@atmosphere_poker_dealer_bot (Vercel Edge)"]
        D -->|"Проверка Whitelist из базы"| E{"Ведущий в базе?"}
        E -->|"Нет"| F["🔒 Вход по Master PIN (7777, имя 'Другое')"]
        E -->|"Да"| G["🎛 Открыть пульт (Telegram WebApp)"]
    end

    subgraph "Прямой веб-доступ и защита"
        H["Прямой переход по ссылке /dealer"] --> I{"Telegram initData или PIN-код?"}
        I -->|"Нет"| J["🔒 Ввод Master PIN ведущего (7777)"]
        I -->|"Да"| K["Пульт ведущего стола (dealer.js)"]
    end

    subgraph "Игровой процесс (Realtime Bus <20ms)"
        K -->|"Старт / Пауза / Таймированный перерыв 2-5-10м / Аут / Ребаланс"| L[("Firebase Realtime Database (europe-west1)")]
        L -->|"Трансляция по WebSocket + REST Fallback (<30ms)"| M["📺 Samsung 75' 4K TV Dashboard (tv.js)"]
    end

    subgraph "Лидерборд и статистика"
        K -->|"Завершение игры"| N["Google Form (SnG / MTT / Mystery) с автозаполнением"]
        N -->|"Запись ответов"| O["Таблица DB_Results"]
        O -->|"Аналитика и очки"| P["Лидерборд (Личный кабинет, Зал славы)"]
        O -->|"Мгновенный пуш события"| L
    end
```

---

## 2. Спецификация турнирных форматов и структур

> Полный официальный паспорт структур и математическая модель зафиксированы в [POKER_STRUCTURES.md](file:///c:/vibe/docs/POKER_STRUCTURES.md).

1. **🏛️ Классика Атмосферы (SNG_DEEP_1500 — 1 500 стек, 10 мин):**
   * Базовая структура клуба без анте.
   * Уровни: 1: 5/10, 2: 10/25, 3: 25/50, 4: 50/100, 5: 100/200, 6: 200/400, 7: 400/800, 8: 800/1600, 9: 1000/2000.
   * Длительность: 65–80 минут.

2. **⚡ Атмосфера Pro (5 000 стек, 7 мин с BBA):**
   * Соревновательная структура со сглаженным переходом (добавлен 400/800 BBA 800) и Color-Up после 150/300.
   * Длительность: 72–78 минут.

3. **Google Формы отправки результатов:**

2. **🏰 SnG Deep (SNG_DEEP_1500 — 1 500 стек, 10 мин):**
   * 0 встроенных перерывов, без анте.
   * Уровни: 1: 5/10, 2: 10/25, 3: 25/50, 4: 50/100, 5: 100/200, 6: 200/400, 7: 400/800, 8: 800/1600, 9: 1000/2000.
   * Google Form: `https://docs.google.com/forms/d/e/1FAIpQLSfCfnN2LS4mAmbQfPtBLZGxPoiYfSqNoaX5xLrmyBr3S5FiEg/viewform`.

3. **🏆 MTT (Многостоловой турнир):**
   * Google Form: `https://docs.google.com/forms/d/e/1FAIpQLSeIDDkj2iCPtMZm-0K5YdZFlopAR7aPfRer2n1o-FQD-Dr7FQ/viewform`.
   * МТТ-движок: учет участников, ауты, умный ребаланс боксов, сводный TV-виджет.

4. **🎯 Mystery Bounty:**
   * Google Form: `https://docs.google.com/forms/d/e/1FAIpQLScFJXRH7bgb2W2aCOeSAKYfL-m4odE14HM5a2eWGz8to4QIlA/viewform`.
   * Учет нокаутов и баунти-очков.

---

## 3. Чеклист задач спринта до планки 9.5/10 (Quality Action Plan)

Каждый блок состоит из строго парных задач: **[ ] TASK-... (Реализация)** $\rightarrow$ **[ ] VERIFY-... (Сквозная E2E проверка и деплой)**.

---

### 🔒 Фаза 1. Безопасность, изоляция секретов и ликвидация спама
* [x] **TASK-1.1:** Изоляция токенов, чистка репозитория и защита от утечек
* [x] **VERIFY-1.1:** Тест безопасности и изоляции секретов (`tests/test_security_sanitization.js`)
* [x] **TASK-1.2:** Гарантированное устранение спама и сброс очереди апдейтов
* [x] **VERIFY-1.2:** Тест тишины бота и защиты от шторма вебхуков (`tests/test_bot_silence_and_dedup.js`)

---

### 👤 Фаза 2. Автоопределение ведущего и чистые структуры игры
* [x] **TASK-2.1:** Бесшовное автоопределение дилера без ручного выбора
* [x] **VERIFY-2.1:** Тест автоопределения ведущего в браузере и Telegram (`tests/test_dealer_identity_guard.js`)
* [x] **TASK-2.2:** Первичное обновление структур (7 мин стандарт и классика)
* [x] **VERIFY-2.2:** Тест структур блайндов и таймингов (`tests/test_structures_and_breaks.js`)

---

### 🏆 Фаза 3. МТТ-движок: ауты, рассадка, ребаланс боксов и ТВ-дашборд
* [x] **TASK-3.1:** Пульт дилера для МТТ: ввод участников, кнопка «Аут» и ребаланс боксов
* [x] **VERIFY-3.1:** Тест МТТ логики аутов и пересадки (`tests/test_mtt_balancing_engine.js`)
* [x] **TASK-3.2:** ТВ-дашборд 4K для МТТ: виджет игроков, средний стек и баннер пересадки
* [x] **VERIFY-3.2:** Визуальный E2E тест МТТ дашборда 4K в браузере (`tests/test_tv_mtt_dashboard.js`)

---

### 🧹 Фаза 4. Чистка мертвого кода, ревизия репозитория и документация
* [x] **TASK-4.1:** Удаление мертвого кода и неиспользуемых артефактов
* [x] **VERIFY-4.1:** Аудит чистоты кодовой базы (`tests/test_config_diagnostics.js`)

---

### 🚀 Фаза 5. Регрессионный сьют и Vercel Deployment
* [x] **TASK-5.1:** Полный сквозной прогон всех тестов
* [x] **VERIFY-5.1:** Конфигурация `vercel.json` и релизный деплой

---

### 💎 Фаза 6. Защита прямого доступа, динамический реестр ведущих, чистые структуры и таймированные паузы
* [x] **TASK-6.1:** Защита пульта от несанкционированного прямого доступа (PIN / WebApp Guard)
* [x] **VERIFY-6.1:** Тест защиты прямого доступа (`tests/test_dealer_security_pin_guard.js`)
* [x] **TASK-6.2:** Динамический реестр ведущих через Google Таблицу и чистка Firebase
* [x] **VERIFY-6.2:** Тест динамического реестра и чистоты Firebase (`tests/test_dynamic_dealers_sync.js`)
* [x] **TASK-6.3:** Обновление турнирных структур (0 перерывов в сетке) и структура 1 500 фишек
* [x] **VERIFY-6.3:** Тест чистых структур 5000 и 1500 фишек (`tests/test_structures_v2.js`)
* [x] **TASK-6.4:** Быстрые таймированные паузы в пульте (2 мин / 5 мин / 10 мин) и ТВ-таймер
* [x] **VERIFY-6.4:** Тест таймированных пауз на пульте и ТВ 4K (`tests/test_timed_pauses.js`)
* [x] **TASK-6.5:** Формы по форматам (SnG/MTT/Mystery) и имя «Другое» для гостей
* [x] **VERIFY-6.5:** Тест маршрутизации форм и гостевой авторизации (`tests/test_forms_and_guest_pin_auth.js`)

---

### ⚡️ Фаза 7. Сетевая надежность, оффлайн-устойчивость и кэширование Leaderboard

#### [x] TASK-7.1. Оффлайн-устойчивость пульта (PendingQueue) и абсолютные тайминги
* **Что делаем:**
  * В `dealer/dealer.js` добавить очередь отложенной отправки запросов (`atmosphere_pending_queue`) в `localStorage` при сбоях связи (в подвале/лифте).
  * Базировать таймер исключительно на абсолютных миллисекундах `levelEndsAt` и `pauseEndsAt`, гарантируя точность до секунды при сворачивании и разворачивании Telegram.
* **Файлы:** `dealer/dealer.js`

#### [x] VERIFY-7.1. E2E Тест оффлайн-очереди и фоновой синхронизации
* **Проверка:** Написать автотест `tests/test_dealer_offline_resilience.js` (эмуляция отключения сети, запись состояний и авто-сброс очереди при восстановлении связи).

---

#### [x] TASK-7.2. Vercel Edge Proxy для лидерборда и мгновенный пуш из таблицы
* **Что делаем:**
  * В `api/leaderboard.js` реализовать легковесный Serverless Proxy к Google Apps Script с заголовками `s-maxage=60, stale-while-revalidate=300` для ультрабыстрого отклика (<50ms) без CORS-ошибок.
  * В `index.html` переключить `DEFAULT_API` на `/api/leaderboard`.
  * В `Code.js` (`processFormSubmit`) добавить триггерный пуш события обновления в Firebase RTDB `/atmosphere/leaderboard_sync.json`.
* **Файлы:** `api/leaderboard.js`, `index.html`, `Code.js`, `FirebaseSync.js`

#### [x] VERIFY-7.2. Тест Edge Proxy и событийного обновления лидерборда
* **Проверка:** Автотест `tests/test_edge_leaderboard_proxy.js` (проверка кэширования, ответа Proxy и формата данных).

---

### 📺 Фаза 8. ТВ-дашборд 4K: Резервный транспорт данных и овертайм-анимации

#### [x] TASK-8.1. Двойной транспорт данных (WebSocket + REST Polling fallback) для Smart TV
* **Что делаем:**
  * В `tv/tv.js` реализовать автоматический fallback на периодический REST-опрос (`fetch('/atmosphere/tables.json')` раз в 1.5 сек), если Firebase JS SDK заблокирован браузером ТВ или отвалился WebSocket.
  * Добавить визуальную пульсацию и ненавязчивый звуковой сигнал в режиме овертайма (когда таймер ушел в минус).
* **Файлы:** `tv/tv.js`, `tv/styles.css`

#### [x] VERIFY-8.1. Тест отказоустойчивости ТВ-дашборда
* **Проверка:** Автотест `tests/test_tv_transport_resilience.js` (симуляция падения WebSocket и успешного переключения на REST).

---

### 🎲 Фаза 9. Умный МТТ ребаланс и исключение выбывших мест

#### [x] TASK-9.1. Исключение пустых боксов при ребалансе столов
* **Что делаем:**
  * В `dealer/dealer.js` генерировать случайный номер бокса только из диапазона реально занятых мест на столе (на основе `table.playersCount`), с возможностью мгновенного реролла.
* **Файлы:** `dealer/dealer.js`

#### [x] VERIFY-9.1. Тест логики ребаланса с учетом живых игроков
* **Проверка:** Автотест `tests/test_mtt_smart_rebalance.js`.

---

### 🏁 Фаза 10. Финальная ревизия, чистка и релиз 9.5/10
* [x] **TASK-10.1:** Полная ревизия кодовой базы, удаление мертвого кода и запуск всех тестов
* [x] **VERIFY-10.1:** Фиксация эталонного качества 9.5/10 (`tests/run_all_tests.js`)

---

### 🏛️ Фаза 11. Турнирный движок TDv3: изоляция столов, точность пауз, авто-уровни и Impeccable TV

#### [x] TASK-11.1. Изоляция записи столов (Per-Table Scoped Writes)
* **Что сделано:** В `dealer/dealer.js` запись переведена со всего корня `/atmosphere/tables` строго на узел своего стола `atmosphere/tables/${DEALER_ID}`. Исключена взаимная перезапись и сброс чужих таймеров.
* **Файлы:** `dealer/dealer.js`

#### [x] VERIFY-11.1. Тест изоляции записи параллельных столов
* **Проверка:** Автотест `tests/test_multi_table_isolation.js` ✅ PASSED (34/34 тестов).

---

#### [x] TASK-11.2. Миллисекундная точность паузы (ликвидация скачка 2:33 ➔ 2:34)
* **Что сделано:** Хранение точного остатка в миллисекундах `table.remainingMs`, ликвидирован рассинхрон `Math.floor` и `Math.ceil`. Возобновление строго с той же миллисекунды. Скачок времени вперед устранен.
* **Файлы:** `dealer/dealer.js`, `tv/tv.js`

#### [x] VERIFY-11.2. Тест точности паузы и монотонности таймера
* **Проверка:** Автотест `tests/test_timer_pause_precision.js` ✅ PASSED (35/35 тестов).

---

#### [x] TASK-11.3. Автоматический транзит уровней блайндов и звуковое оповещение
* **Что сделано:** По истечении времени уровня (`now >= levelEndsAt`) раунд автоматически переключается на следующий без ручного вмешательства дилера. Реализован встроенный Web Audio API Chime (двухтональный гонг смены блайндов), работающий автономно без внешних mp3-файлов.
* **Файлы:** `dealer/dealer.js`, `tv/tv.js`

#### [x] VERIFY-11.3. Тест автоперехода уровней без участия дилера
* **Проверка:** Автотест `tests/test_auto_level_progression.js` ✅ PASSED (36/36 тестов).

---

#### [x] TASK-11.4. Редизайн ТВ-дашборда по стандартам Impeccable (1, 2, 3, 4 стола)
* **Что сделано:** Полная ликвидация мертвого пространства. Внедрена монументальная иерархия шрифтов (для 1 стола блайнды до 110px, таймер до 230px; для 2 столов блайнды 68px; для 3 столов 52px; для 4 столов 46px вместо старых 26px). Высококонтрастный цвет следующего уровня (#cbd5e1), неоновое золотое свечение текущих блайндов.
* **Файлы:** `tv/styles.css`, `tv/tv.js`

#### [x] VERIFY-11.4. Визуальный E2E тест 4K ТВ-дашборда
* **Проверка:** Автотест `tests/test_tv_impeccable_layouts.js` ✅ PASSED (37/37 тестов).

---

### ФАЗА 12: Оптимизация ТВ-производительности, Web Audio Countdown, Hotkeys & MTT Ребаланс

#### [x] TASK-12.1. DOM-Patching в tv.js и сетевой пинг-индикатор
* **Что сделано:** Устранена деструктивная перезапись `innerHTML` каждые 250 мс. Внедрен адаптивный DOM-Patching: карточки столов создаются один раз, а в тиках обновляются только символы цифр и классы состояний (нагрузка на CPU Smart TV снижена до ~0%). В шапку ТВ добавлен бейдж сетевого пинга (WS / REST).
* **Файлы:** `tv/tv.js`, `tv/index.html`, `tv/styles.css`

#### [x] VERIFY-12.1. Тест DOM-Patching, среднего стека в BB и пинг-индикатора
* **Проверка:** Автотест `tests/test_tv_dom_patching_and_ping.js` ✅ PASSED (38/40 тестов).

---

#### [x] TASK-12.2. Web Audio 5-секундный отсчет, AudioContext Unlock и горячие клавиши ТВ
* **Что сделано:** Процедурный синтез предупредительных тиков на 5, 4, 3, 2, 1 секундах перед сменой блайндов (600–720 Гц) с мягкой золотой пульсацией карточки стола. Внедрен оверлей разблокировки AudioContext при первом клике. Добавлены горячие клавиши на ТВ: `Space` (пауза/старт), `N` (след. уровень), `P` (пред. уровень), `+`/`-` (коррекция времени на 60с), `F`/`Enter` (полный экран).
* **Файлы:** `tv/tv.js`, `tv/index.html`, `tv/styles.css`

#### [x] VERIFY-12.2. Тест Web Audio отсчета и горячих клавиш
* **Проверка:** Автотест `tests/test_audio_countdown_and_hotkeys.js` ✅ PASSED (39/40 тестов).

---

#### [x] TASK-12.3. Автоматический Color-Up после 100/200 и атомарный ребаланс МТТ
* **Что сделано:** Строго после уровня 4 (`100/200`) перед переходом на `150/300` автоматически запускается 2-минутный перерыв для размена фишек <100 (Color-Up). В пульт дилера добавлена кнопка «Пропустить Color-Up» для мгновенного перехода к 150/300. В МТТ при подтверждении ребаланса (`confirmRebalance`) выполняется атомарное сетевое обновление `playersCount` целевого стола в Firebase без риска затирания чужого таймера.
* **Файлы:** `dealer/dealer.js`, `dealer/index.html`, `dealer/styles.css`, `tv/tv.js`

#### [x] VERIFY-12.3. Тест Color-Up и атомарного ребаланса МТТ
* **Проверка:** Автотест `tests/test_colorup_and_mtt_atomic_rebalance.js` ✅ PASSED (40/40 тестов).

---

## 4. Протокол работы автономного агента (Autonomous Loop Rules)

1. **Строгая попарность:** Исполнитель берет ровно одну незакрытую задачу `[ ] TASK-...`, реализует её, затем **ОБЯЗАТЕЛЬНО** берет парную `[ ] VERIFY-...` и запускает автотест.
2. **Никакой грязи в Git:** После каждого выполненного шага делается Git коммит и push.
3. **Обновление статуса:** Выполненная задача отмечается как `[x]` в `ORCHESTRATOR.md`.
4. **Контроль планки 9.5/10:** Полная согласованность фронтенда, бэкенда, Firebase и Google Таблиц.
